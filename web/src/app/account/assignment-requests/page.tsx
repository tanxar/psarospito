...
type IncomingAssignmentRequest = {
  id: string;
  createdAt: string;
...
export default function AssignmentRequestsPage() {
  ...
  const [items, setItems] = useState<IncomingAssignmentRequest[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/brokers/incoming-assignment-requests", { cache: "no-store" })
      .then(async (r) => {
        if (!r.ok) {
      ...
      .then((data: unknown) => {
        if (cancelled) return;
        setItems(Array.isArray(data) ? (data as IncomingAssignmentRequest[]) : []);
      })
      .catch((err: unknown) => {
        if (cancelled) return;
        ...
        <div className="mx-auto w-full max-w-4xl px-4 pb-14 pt-8 sm:px-6 lg:px-8">
          <Card className="rounded-xl border-border bg-card">
            <CardContent className="py-10 text-center text-sm text-muted-foreground">
              Συνδέσου για να δεις τα εισερχόμενα αιτήματα ανάθεσης.
            </CardContent>
          </Card>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto w-full max-w-4xl px-4 pb-14 pt-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-5 border-b border-border pb-5">
          <h1 className="text-2xl font-semibold text-foreground sm:text-3xl">Αναθέσεις μεσίτη · Εισερχόμενα</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Ιδιώτες που ζήτησαν να αναλάβεις αγγελία. Επόμενο βήμα: αποδοχή ή απόρριψη από τη σελίδα αγγελίας.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <Card className="mt-6 rounded-xl border-border bg-card">
            <CardContent className="py-12 text-center text-sm text-muted-foreground">
              Δεν υπάρχουν νέα αιτήματα ανάθεσης.
            </CardContent>
          </Card>
        ) : (
            ...
            {items.map((req) => (
              ...
              >
                <div className="min-w-0">
                  <p className="truncate font-medium text-foreground">{req.listing.title}</p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Ο/Η {req.owner.name} ζήτησε να αναλάβεις την αγγελία.
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">Αίτημα: {formatDate(req.createdAt)}</p>
                </div>
