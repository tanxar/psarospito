...
export default function SentOffersPage() {
  ...
  const [items, setItems] = useState<SentOffer[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/brokers/sent-offers", { cache: "no-store" })
      .then(async (r) => {
        if (!r.ok) {
      ...
      .then((data: unknown) => {
        if (cancelled) return;
        setItems(Array.isArray(data) ? (data as SentOffer[]) : []);
      })
      .catch((err: unknown) => {
        if (cancelled) return;
        ...
        <div className="mx-auto w-full max-w-4xl px-4 pb-14 pt-8 sm:px-6 lg:px-8">
          <Card className="rounded-xl border-border bg-card">
            <CardContent className="py-10 text-center text-sm text-muted-foreground">
              Συνδέσου για να δεις τα απεσταλμένα αιτήματα ανάθεσης.
            </CardContent>
          </Card>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto w-full max-w-4xl px-4 pb-14 pt-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-5 border-b border-border pb-5">
          <h1 className="text-2xl font-semibold text-foreground sm:text-3xl">Αναθέσεις μεσίτη · Απεσταλμένα</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Αιτήματα ανάληψης που έχεις στείλει σε ιδιώτες και βρίσκονται σε αναμονή.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <Card className="mt-6 rounded-xl border-border bg-card">
            <CardContent className="py-12 text-center text-sm text-muted-foreground">Δεν υπάρχουν ενεργές προσφορές που έστειλες.</CardContent>
          </Card>
        ) : (
            ...
            {items.map((item) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">Προς: {item.owner.name}</p>
                  {item.message ? <p className="mt-1 text-xs text-muted-foreground">Μήνυμα: {item.message}</p> : null}
                  <p className="mt-1 text-xs text-muted-foreground">Αποστολή: {formatDate(item.createdAt)}</p>
                </div>
